...
class Sequence(Dataset):
    ...
    def __init__(self, seq_path: Path, mode: str = 'train', event_representation: str = 'voxel_grid',
        ...
        if self.augmentation:
            ...
            self.transform_a_random_crop = A.ReplayCompose([
                A.RandomScale(scale_limit=(0.02, 0.2), p=1.0),
                A.RandomCrop(height=self.shape_crop[0], width=self.shape_crop[1]),
                A.HorizontalFlip(p=0.5),
            ])
        else:
            self.transform_a = None
            self.transform_a_random_crop = None

        # Deterministic validation/test transform -> always output 448x448
        self.transform_a_center_crop = A.ReplayCompose([
            A.Resize(height=self.shape_crop[0], width=self.shape_crop[1]),
        ])
    ...
    def apply_augmentation(self, transform_a, events, images, label):
        if self.require_paired_data:
            ...
            out_h, out_w = A_data['image'].shape[:2]
            events_tensor = torch.zeros((events.shape[0], out_h, out_w), dtype=events.dtype)

            for k in range(events.shape[0]):
                replayed = A.ReplayCompose.replay(
                    A_data['replay'],
                    image=events[k, :, :].numpy()
                )['image']
                events_tensor[k, :, :] = torch.from_numpy(replayed)

            return events_tensor, img_tensor, label

        else:
            A_data = transform_a(
                image=events[0, :, :].numpy(),
                mask=label
            )
            label = A_data['mask']
    ...
    def __getitem__(self, index):
        ...
        for location in self.locations:
            ...
            # Remove bottom 40 rows: 480x640 -> 440x640
        ...
        if self.mode == 'train' and self.random_crop:
            if self.require_paired_data:
                event_tensor, img_tensor, label = self.apply_augmentation(
                    self.transform_a_random_crop, event_tensor, img_tensor, label
                )
            else:
                event_tensor, label = self.apply_augmentation(
                    self.transform_a_random_crop, event_tensor, img_tensor, label
                )
            ...
            if self.require_paired_data:
                event_tensor, img_tensor, label = self.apply_augmentation(
                    self.transform_a_center_crop, event_tensor, img_tensor, label
                )
            else:
                event_tensor, label = self.apply_augmentation(
                    self.transform_a_center_crop, event_tensor, img_tensor, label
            ...
            if self.augmentation and self.transform_a is not None:
                if self.require_paired_data:
                    event_tensor, img_tensor, label = self.apply_augmentation(
                        self.transform_a, event_tensor, img_tensor, label
                    )
                else:
                    event_tensor, label = self.apply_augmentation(
                        self.transform_a, event_tensor, img_tensor, label
